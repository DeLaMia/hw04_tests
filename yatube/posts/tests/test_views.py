...
class PostVievsTests(TestCase):
    ...
    def test_first_page_contains_ten_records(self):
        ...
        pages_names = {
            ...
            reverse('posts:group_list', kwargs={'slug': 'test-slug'}): POSTS_WITH_GROUP,
            reverse('posts:profile', kwargs={'username': 'NoName'}): POSTS_AUTHOR_USER,
        }
        ...
        for reverse_name, post_count in pages_names.items():
            with self.subTest(reverse_name=reverse_name):
                ...
                self.assertEqual(len(response.context['page_obj']), post_count - POST_IN_PAGE) 
        
    def test_post_detail_pages_show_correct_context(self):
        """Шаблон post_detail сформирован с правильным контекстом."""
        response = (self.authorized_client.get(reverse('posts:post_detail', kwargs={'post_id': self.post.id})))
        self.assertEqual(response.context.get('post_more').author.username, 'NoName')
        self.assertEqual(response.context.get('post_more').text, 'test-post-text')
        self.assertEqual(response.context.get('post_more').group.title, 'test-text') 
